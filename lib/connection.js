...
function setReadResponse(data) {
    return data || { items: {}, ids: [] };
}
 ...
 *
 * @param {object} options - The options for this Connection instance
 */
function Connection(options) {

    EventEmitter.call(this);

    var opts = misc.defaults(options, defaultOpts.connectionDefaults());
...
Connection.prototype.connect = function() {
    ...
    // Ryan Rubley - all memcached commands use exlicitly only \r\n but carrier will split on \r?\n by default
    // We need to specify exactly '\r\n' here so carrier does not mangle VALUE responses which contain just \n in the data block
    carrier.carry(self.client, self.read.bind(self), 'utf-8', '\r\n');
};
...
Connection.prototype.read = function(data) {
    ...
    // Ryan Rubley - This should never happen, so it's just here as a safety net now
    // It was happening in old versions of memcache-plus/memcache-pp after things desynced due to ignoring 0-length data lines
    if (!deferred) {
        debug('not expecting any reply, dropping data');
        return;
    }
    ...
    // Ryan Rubley - if we're expecting a data block (after a VALUE line) then we should not be testing data against any other
    // special command. If the data block itself happens to contain \r\n, carrier will still split it up, and we will reassemble
    // it here. Once all the expected bytes are read in, we will transfer everything over to completedValue
    // This also handles get/gat with multiple keys returning multiple VALUE lines before a single END
    if (this.expectValueKey != null) {
        if (this.data != null) {
            resp = this.data + '\r\n' + data;
            this.expectValueRemainingBytes -= 2;
        } else {
            resp = data;
        }
        // Ryan Rubley - we need the length in bytes, not the length in UTF-16 codepoints
        this.expectValueRemainingBytes -= Buffer.from(data).length;
        if (this.expectValueRemainingBytes <= 0) {
            this.expectValueResp[0] = resp;
            if (Array.isArray(deferred.key)) {
                if (!this.completedValue) {
                    this.completedValue = {};
                }
                this.completedValue[this.expectValueKey] = this.expectValueResp;
            } else {
                this.completedValue = this.expectValueResp;
            }
            this.expectValueKey = null;
            this.expectValueResp = null;
            this.expectValueRemainingBytes = null;
            resp = null;
        }
        done = false;
    } else if (deferred.type === 'autodiscovery') {
        ...
        // Ryan Rubley - key will now be an array of keys for getMulti/gatMulti
        err = new Error(util.format('Memcache returned an error: %s\r\nFor key %s', data, Array.isArray(deferred.key) ? deferred.key.join(' ') : deferred.key));
    } else if (data.match(/^VALUE .+/)) {
        var spl = data.match(/^VALUE ([^ ]+) ([0-9]+) ([0-9]+)( ([0-9]+))?$/);
        debug('Got some metadata', spl);
        // Ryan Rubley - Instead of keeping this in a shared global variable, let's pre-assemble the response we'll need
        // later, except the data will start out null here, and get filled in once the entire data block is read
        this.expectValueKey = spl[1];
        this.expectValueResp = [ null, Number(spl[2]), Number(spl[3]), spl.length >= 6 ? spl[5] : undefined ];
        this.expectValueRemainingBytes = this.expectValueResp[2];
        done = false;
        // Ryan Rubley - clear this out so that we never resolve the value's data as the literal string "VALUE key 1 2 3"
        resp = null;
    } else if (data.match(/^END$/)) {
        if (deferred.type === 'items') {
            resp = this.data;
        } else if(this.completedValue) {
            // Ryan Rubley - this will be an object for getMulti/gatMulti or just a single array of 3-4 items for get/gat
            resp = this.completedValue;
        ...
        resp = setReadResponse(this.data);
        ...
        if (data.match(/^(STORED|NOT_STORED|DELETED|EXISTS|TOUCHED|NOT_FOUND|OK|INCRDECR|STAT)$/)) {
            ...
            // Ryan Rubley - I have no idea why there was a `if (data !== '') {` here before
            // All it seemed to do was break cases of getting a value that is a 0-length string
...
Connection.prototype.gets = function(key, opts) {
    ...
    return deferred.promise
        ...
        .spread(function(data, flag, length, cas) {
            if (data != null) { // Ryan Rubley - added check against null since an empty string is valid
...
/**
 * getMulti() - Get multiple values for the provided array of keys on this connection
 *
 * @param {Array} keys - The keys to get
 * @param {Object} [opts] - Any additional options for this get
 * @returns {Promise}
 */
 Connection.prototype.getMulti = function(keys, opts) {
    opts = opts || {};
    // Do the get
                ...
                if (value != null) {
                    ...
                    if (data != null) {
                        if (opts.compressed || (flag & FLAG_COMPRESSED)) {
                            // @todo compressed data should still be able to utilize the
                            //  flags to return data of same type set
                            values[key] = misc.decompress(Buffer.from(data, 'base64'))
                                .then(function(d) {
                                    return d.toString();
                                })
                                .catch(function(err) {
                            ...
                            values[key] = convert(data, flag, length);
                        }
                    } else {
                        values[key] = null;
                    }
...
/**
 * touch() - Touch a key on this connection
 */
 Connection.prototype.touch = function(key, ttl) {
    ttl = ttl || 0;
    var opts = {};
    ...
    this.write(util.format('touch %s %d', key, ttl));
...
/**
 * gats() - Get And Touch a value and CID on this connection
 *
 * @param {String} key - The key for the value to retrieve
 * @param {Number|Object} [ttl = 0] - The time to live for this key
 * @param {Object} [opts] - Any additional options for this gat
 * @returns {Promise} - containing an array with the value and CID
 */
 Connection.prototype.gats = function(key, ttl, opts) {
    ttl = ttl || 0;
    opts = opts || {};
    ...
    this.write(util.format('gats %d %s', ttl, key));
...
/**
 * gat() - Get And Touch a value on this connection
 *
 * @param {String} key - The key for the value to retrieve
 * @param {Number|Object} [ttl = 0] - The time to live for this key
 * @param {Object} [opts] - Any additional options for this gat
 * @returns {Promise}
 */
Connection.prototype.gat = function(key, ttl, opts) {
    ttl = ttl || 0;
    opts = opts || {};
    ...
    this.write(util.format('gat %d %s', ttl, key));
...
/**
 * gatMulti() - Get And Touch multiple values for the provided array of keys on this connection
 *
 * @param {Array} keys - The keys to gat
 * @param {Number|Object} [ttl = 0] - The time to live for this key
 * @param {Object} [opts] - Any additional options for this gat
 * @returns {Promise}
 */
 Connection.prototype.gatMulti = function(keys, ttl, opts) {
    ttl = ttl || 0;
    opts = opts || {};
    ...
    this.write(util.format('gat %d %s', ttl, keys.join(' ')));
